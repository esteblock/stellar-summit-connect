...
const TEXT_FIELDS = {
  name: 120, role: 120, project: 160, oneLiner: 280, category: 60,
  profileType: 20, country: 80, city: 80, x: 80, telegram: 80,
  email: 160, lookingFor: 1200,
};
...
function cleanPerson(body) {
  ...
  for (const [field, max] of Object.entries(TEXT_FIELDS)) {
    const v = body[field];
    if (typeof v === 'string') p[field] = v.trim().slice(0, max);
  }
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (route === 'POST /api/tries') {
      ...
      const to = state.people.find((x) => x.id === body.to);
      if (!from || !to) return sendJson(res, 404, { error: 'person not found' });
      if (from.id === to.id) return sendJson(res, 400, { error: 'cannot try your own project' });
      if (body.token !== from.token) return sendJson(res, 403, { error: 'wrong token' });
      const comment = typeof body.comment === 'string' ? body.comment.trim().slice(0, 280) : '';
      const existing = state.tries.find((t) => t.from === from.id && t.to === to.id);
      if (existing) {
        if (comment) existing.comment = comment;
      } else {
        state.tries.push({ from: from.id, to: to.id, comment, at: new Date().toISOString() });
      }
      persist();
      return sendJson(res, 201, { ok: true });
    }
